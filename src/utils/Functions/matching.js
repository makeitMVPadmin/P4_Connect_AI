//retrieving users:
import { getAllUserAnswers, readData } from "./functions";
...
export async function findBestMatch(newUser) {
  try {
    ...
    const usersForMatching = await readData("UserAnswers"); // temporary, until the getalluseranswers functions works

    usersForMatching.forEach((user) => {
      if (user.user_id !== newUser.user_id) {
        const similarity = weightedJaccardSimilarity(
          newUser.answers,
          user.answers
        );
